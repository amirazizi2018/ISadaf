...
console.log(jobseeker.job_seeker.id);
    var dataremove = {
      id: parseInt(jobseeker.job_seeker.id),
    }
  ...
  $scope.GetSendMessage = function() {
    ...
    $http.get(mustafasite + '/employer/sent_message?per_page=100000', config).then(function(response) {
  ...
  $scope.sendEmail = function() {
    ...
    if ($scope.atachmailmodel == null) {
      ...
      $http.post(mustafasite + '/employer/message', JSON.stringify(data), config).then(function(response) {
        $scope.GetSendMessage();
        $scope.composeEmail = {};
  ...
  $scope.GetReceiveMessage = function() {
    ...
    $http.get(mustafasite + '/employer/receive_message?per_page=100000', config).then(function(response) {
  ...
  $scope.showreceivemail = function(email) {
    ...
    if (email.seen == true) {
      ...
      $http.get(mustafasite + '/employer/message/' + email.id, config).then(function(response) {
        $scope.isreceiveVisible = true;
        $scope.GetReceiveMessage();
        $scope.selectedreceiveEmail = email;
      });
  ...
  $scope.removereceiveemail = function(selectedreceiveEmail) {
    ...
    $http.delete(mustafasite + '/employer/message/' + selectedreceiveEmail.id, config).then(function(response) {
      $scope.GetReceiveMessage();
      $scope.isreceiveVisible = false;
    });
  ...
  $scope.removesentmail = function(selectedsentEmail) {
    ...
    $http.delete(mustafasite + '/employer/message/' + selectedsentEmail.id, config).then(function(response) {
      $scope.GetSendMessage();
      $scope.issentVisible = false;
    });
  ...
  $scope.showajs = function(jobseeker) {
	...
	$scope.sendmailtojs = function(jobseeker) {

		$scope.activeTabF = "tab7";

		$scope.activeTab = "compose";

		$scope.composeEmail = {};

		var ShowNameMailReplay = jobseeker.job_seeker.fa_name;

		var input = $('#_value');

		input.val(ShowNameMailReplay);

		$scope.sentuserid = jobseeker.job_seeker.id;
